...
def test_os_listdir_dispatches_to_native_helper():
    ...
    ir_text = _compile_to_ll(program, "native_os_listdir_ir", mode="on")
...
def test_native_os_listdir_runtime_lists_directory(tmp_path):
    ...
    data_dir.mkdir()
    (data_dir / "one.py").write_text("", encoding="utf-8")
    (data_dir / "two.txt").write_text("", encoding="utf-8")
    (data_dir / "three.py").write_text("", encoding="utf-8")
    ...
    src.write_text(
        textwrap.dedent(
            ...
            def main() -> None:
                total = 0
                for name in os.listdir({str(data_dir)!r}):
                    if name.endswith(".py"):
                        total = total + 1
                print(total)
