"""Native next(<genexpr>, default) lowering."""
...
def test_native_next_genexpr_runtime_round_trip(tmp_path):
    from pcc.py_frontend.pipeline import compile_python
    ...
    src.write_text(
        textwrap.dedent(
            """
            def main() -> None:
                print(next((x for x in [5, 6]), 0))
                print(next((x for x in []), 9))
