...
def test_d3_async_runtime_wiring_stays_intact():
    header = Path("pcc/py_runtime/include/py_runtime.h").read_text()
    abi = Path("pcc/py_frontend/codegen/runtime_abi.py").read_text()
    c_src = Path("pcc/py_runtime/src/py_coroutine.c").read_text()
    py_src = Path("pcc/py_runtime/py/py_coroutine.py").read_text()
...
def test_d4_d5_d6_runtime_wiring_stays_intact():
    ...
    protocol = Path("pcc/py_runtime/src/py_protocol.c").read_text()
