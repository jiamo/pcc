...
def _explicit_collect_sweep_probe() -> str:
    return """
        ...
        def main() -> None:
            live = pcc_gc_alloc(24, 2, 0)
            dead = pcc_gc_alloc(24, 2, 0)
            frame_map = malloc(4)
            slots = malloc(8)
...
def test_incremental_backend_env_selects_default_and_small_alloc_is_not_a_step(tmp_path):
    exe = _compile_probe(
        tmp_path,
        """
        from pcc.extern import extern, c_int32, c_int64, c_ptr, c_void
...
def test_incremental_backend_collects_container_churn_under_pcc_python_runtime(tmp_path):
    exe = _compile_probe(
        tmp_path,
        """
    ...
    result = _run_with_backend_one(exe, tuned=False)
...
def test_incremental_backend_explicit_collect_sweeps_with_live_roots(tmp_path):
    exe = _compile_probe(tmp_path, _explicit_collect_sweep_probe())
    result = _run_with_backend_one(exe)
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip().splitlines() == ["True", "0"]


def test_incremental_backend_pcc_python_explicit_collect_sweeps_with_live_roots(
    tmp_path,
):
    exe = _compile_probe(
        tmp_path,
