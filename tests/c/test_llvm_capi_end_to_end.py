...
def test_call4_i32_emits_raw_i32_operand():
    m = pcc_ir.Module("t")
    void = pcc_ir.VoidType()
    i8p = pcc_ir.PointerType(pcc_ir.IntType(8))
    i32 = pcc_ir.IntType(32)
    sink_ty = pcc_ir.FunctionType(void, [i8p, i8p, i8p, i32])
    sink = pcc_ir.Function(m, sink_ty, name="sink")
    fn_ty = pcc_ir.FunctionType(void, [])
    fn = pcc_ir.Function(m, fn_ty, name="caller")
    b = pcc_ir.IRBuilder(fn.append_basic_block("entry"))
    null = pcc_ir.Constant(i8p, None)
    pcc_ir.IRBuilder_call4_i32(b, sink, null, null, null, 123)
    b.ret_void()

    text = str(m)
    assert "i32 123" in text
    parsed = pcc_bind.parse_assembly(text)
    parsed.verify()
