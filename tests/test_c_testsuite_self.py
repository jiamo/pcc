...
pytestmark = pytest.mark.xdist_group(name="c_testsuite_self")


def _run_self_backend(case_path: Path, timeout: int | None = None) -> PccCompileResult:
    return _run_backend(case_path, backend="self", timeout=timeout, allow_unimplemented_backend=True)
...
@pytest.mark.parametrize("filename", C_TESTSUITE_SELF_BACKEND_EXACT_MATCH_CASES)
def test_c_testsuite_self_backend_runtime_matches_native_exactly(filename):
    ...
    assert_result_triplet_matches(filename, "self", self_result, "native", native_result)


@pytest.mark.parametrize("filename", C_TESTSUITE_SELF_BACKEND_DIFFERENTIAL_CASES)
...
@pytest.mark.parametrize("filename", C_TESTSUITE_SELF_BACKEND_CASES_WITH_EXPECTED_OUTPUT)
