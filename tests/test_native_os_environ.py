...
def test_os_environ_subscript_still_falls_back_off_mode():
    """``os.environ[X]`` is a different shape (subscript, not method)
    and is not yet covered by the native dispatch — confirm it still
    falls through to the dynamic path so we know the dispatch is
    targeted, not over-aggressive.
    """
    program = textwrap.dedent(
        """
        import os
    ...
    assert body is not None
    # subscript path is not in scope for this change; expect it to keep
    # routing through the dynamic getitem
