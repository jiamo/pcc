...
Currently the pcc-C archive only covers the no-libpython path. Programs
whose IR still emits py_cpy_* fallback calls transparently reuse the
cc-built libpython archive even under PCC_RUNTIME_CC=pcc, because pcc
cannot yet compile py_libpython.c against CPython's headers (opaque
struct loads like _Py_NoneStruct). Phase 4 makes this moot by
migrating runtime-high to Python and removing py_cpy_* call sites.
"""
...
def _compile_and_run(
    ...
    compile_result = subprocess.run(
        [pcc_bin, "--verbose", str(source), "-o", str(out_path)],
        capture_output=True,
        text=True,
        timeout=120,
        cwd=str(REPO_ROOT),
