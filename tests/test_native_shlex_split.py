...
def test_shlex_split_dispatches_to_native_helper():
    program = textwrap.dedent(
        r'''
        import shlex

        def f():
            return shlex.split('one "two three" four\ five')
        '''
    )

    ir_text = _compile_to_ll(program, "native_shlex_split_ir", mode="on")
...
def test_native_shlex_split_runtime_handles_quotes_and_escapes(tmp_path):
    from pcc.py_frontend.pipeline import compile_python
    ...
    src.write_text(
        textwrap.dedent(
            ...
            def main() -> None:
                parts = shlex.split('one "two three" four\ five')
                print(len(parts))
                print(parts[1])
                print(parts[2])
