# Saved full-context regression for PY-P1-CONTEXTUAL-CLASS-METHOD-ARG-TAGGING.
# Keep this source independent of the evolving product module: the surrounding
# imports, globals, and functions are part of the historical lowering context.
...
class ContextualAnimApp:
    # Keep the business values contextual so this saved failure exercises the
    # pointer-form method ABI that exposed tagged values at the extern edge.
    def anim_start(
        self,
        from_value,
        to_value,
    ...
    def exercise(self) -> None:
        anim = stack_alloc(40)
        _setg(72, ptr_to_int(anim))
        rc = self.anim_start(0, 100, 2000)
        print(rc)
        print(load_i64(anim, 0))
        print(load_i64(anim, 8))
        print(load_i64(anim, 16))
        print(load_i64(anim, 24))
        print(load_i32(anim, 32))


ContextualAnimApp().exercise()
