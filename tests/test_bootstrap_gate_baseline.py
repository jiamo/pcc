...
- ``otool -L`` libpython linkage state matches baseline (currently
  ``true`` for every binary; Path A's success flips this to ``false``).
- pcc2 and pcc3 are byte-identical after Mach-O signature
  normalization (the README's three-stage self-host gate).

When Path A migrations land:
- ``links_libpython`` should transition true → false per binary.
- Update ``bootstrap_gate_baseline.json`` to reflect the new state.
- ``byte_identical_pcc2_pcc3`` must remain ``true`` throughout.
"""
