...
from tests.gcc_torture_cases import DEFAULT_TIMEOUT, PccCompileResult, _read_case_source, run_native
...
pytestmark = pytest.mark.xdist_group(name="gcc_torture_self")
...
def _run_llvm_backend(case_path: Path, timeout: int = DEFAULT_TIMEOUT):
    return _run_backend(case_path, backend="llvm", timeout=timeout)


@pytest.mark.parametrize("relative_path", GCC_TORTURE_SELF_BACKEND_EXACT_MATCH_CASES)
def test_gcc_torture_self_backend_runtime_matches_native_exactly(relative_path):
    ...
    assert_result_triplet_matches(relative_path, "self", self_result, "native", native_result)


@pytest.mark.parametrize("relative_path", GCC_TORTURE_SELF_BACKEND_DIFFERENTIAL_CASES)
def test_gcc_torture_self_backend_matches_llvm_on_exact_match_bucket(relative_path):
    ...
    assert_result_triplet_matches(relative_path, "self", self_result, "llvm", llvm_result)


@pytest.mark.parametrize("relative_path", GCC_TORTURE_SELF_BACKEND_RETURNCODE_CASES)
