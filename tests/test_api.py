...
class TestBuild:
    ...
    def test_build_self_backend_raises(self, simple_c):
        from pcc import build

        with pytest.raises(ValueError, match="backend 'self' is selected but not implemented"):
            build(simple_c, kind="sharedlib", backend="self")
