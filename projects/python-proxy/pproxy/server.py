...
def forget_channel_task(task):
    try:
        ACTIVE_CHANNEL_TASKS.remove(task)
    except ValueError:
        pass

def close_active_channel_writers():
    writers = list(ACTIVE_CHANNEL_WRITERS)
    del ACTIVE_CHANNEL_WRITERS[:]
...
def link_channel_tasks(channels, writers):
    tasks = [asyncio.ensure_future(channel) for channel in channels]
    ACTIVE_CHANNEL_TASKS.extend(tasks)
    ACTIVE_CHANNEL_WRITERS.extend(writers)
    def stop_peer(done_task):
        forget_channel_task(done_task)
        for task in tasks:
...
def pending_tasks(loop):
    return [
        task for task in (
            asyncio.all_tasks(loop) if hasattr(asyncio, 'all_tasks') else asyncio.Task.all_tasks()
        )
        if not task.done()
    ]

def cleanup_run_until_complete(loop, awaitable):
    try:
        return loop.run_until_complete(awaitable)
    except KeyboardInterrupt:
        return None

def drain_pending_tasks(loop):
    close_active_channel_writers()
    for _ in range(3):
        tasks = pending_tasks(loop)
        if not tasks:
            return
        for task in tasks:
            task.cancel()
        cleanup_run_until_complete(loop, asyncio.gather(*tasks, return_exceptions=True))
        cleanup_run_until_complete(loop, asyncio.sleep(0))
...
def main(args = None):
    ...
    if args.sslfile:
        ...
        return
    if args.test:
        get_or_create_event_loop().run_until_complete(test_url(args.test, args.rserver))
        return
