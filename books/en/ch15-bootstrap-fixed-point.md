...
-m MODULE ...                       → _run_host_python_module_from_pcc1
   pip / pcc.package.* among them   →   native package shims
                                        (_run_native_pip_shim_from_pcc1 etc.,
                                         see Chapter 17)
   any other module                 →   subprocess to PCC_HOST_PYTHON
                                        (default python3)
...
When the fixed-point gate goes red, the first action is not to fix — it is to classify. The "If pcc1 and pcc2 differ" section of [codex-goal-prompt.md](../../codex-goal-prompt.md) defines nine classes and attaches one prohibition: **until the difference class is identified, do not patch around the symptom.**
...
3. **Trace the closure**: `_run_host_python_module_from_pcc1()` in [pcc/cli_bootstrap.py](../../pcc/cli_bootstrap.py) uses `["env", "PYTHONPATH=" + os.getcwd(), host, "-m", module_name]` rather than `subprocess.run(..., env=...)`. Combining the comment at that site with the fallback routing of Chapter 14, explain which lowering path the `env=` keyword would traverse to trigger the libpython fallback, and why the external `env` command is the closure-safe equivalent.
