...
**`del_method`.** Finalizer lookup sits on the dealloc hot path, so it is cached at a fixed offset: `py_class_new()` pre-fills it at the end with `py_class_lookup(c, "__del__")` (which also finds inherited finalizers); `py_class_add_method()` updates the cache when it sees `"__del__"`; and `py_user_del_dispatch()` in `py_dunder.c` lazily fills the slot once if it finds it empty. Notably, the pcc-Python port's `py_class_new` does *not* do the pre-fill (memset leaves NULL); the two implementations converge to the same observable behavior via the lazy fill. This is a clean example of "the layout must be byte-identical; the behavior may differ in pacing, but must converge" (Exercise 3).
...
## 7.5 One Layout, Two Implementations: The C ↔ pcc-Python Mirroring Discipline

pcc's runtime layering (Chapters 1 and 14) requires Python semantics to migrate, step by step, out of handwritten C and into pcc-Python: [pcc/py_runtime/src/py_class.c](../../pcc/py_runtime/src/py_class.c) has a twin, [pcc/py_runtime/py/py_class.py](../../pcc/py_runtime/py/py_class.py), which exports **the same symbols with the same ABI** via decorators like `@c_abi_export("py_class_lookup")`, and the default build (`PY_MODULES`) links the port, not the C source. The same `PyClassObject` therefore has two producers and two consumers, and the layout becomes **shared property** between two implementations.
...
3. **Behavior may differ convergently; layout may not differ at all.** Pre-filling `del_method` (C) versus lazy filling (port) is a legitimate difference, because the observable ABI behavior is identical; offset 96 versus offset 88 is not a difference, it is corruption.
...
3. **(Convergence proof)** The C `py_class_new()` pre-fills `del_method`; the port does not. Using the lazy-fill logic in `py_user_del_dispatch()`, prove that the two are indistinguishable for any user program; then construct a point of difference (observable only with runtime-internal probes) and explain why "observable ABI equivalence" is a sounder mirroring standard than "statement-for-statement equivalence."
