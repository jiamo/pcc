...
3. **The runtime is consumed by both pipelines.** The Python path links it as an archive (`libpy_runtime*.a`); and when `PCC_RUNTIME_CC=pcc`, the runtime's C sources are themselves compiled by the C frontend — the C frontend's maturity directly determines how much the runtime archive can be trusted.
...
| [pcc/py_runtime/](../../pcc/py_runtime) | Runtime: C sources + pcc-Python ports + five GCs (Chapters 7–11, 14) |
