...
3. **诚实是文体的一部分。** pcc 的声明卫生(mode-labeled claims)同样约束本书:
   ...
   - 实验性的东西明说实验性;已知缺陷(如 typed-int 溢出洞)如实写成开放问题。
4. **战争故事是一等内容。** 每章设"历史与教训"一节,取材 `docs/investigations/`
   的真实调查(带日期),讲清:症状 → 错误假设 → 证据链 → 真正根因 → 留下的不变式。
   这是本书区别于教科书的地方。
...
本章导言(为什么有这一章,150-300字)
N.1 问题与设计空间(为什么)
N.2 ... N.x 机制(怎么做,落在源码上)
N.y 历史与教训(investigations 战争故事,≥2 个)
N.z 小结
练习(3-5 题,从"读源码验证"到"设计权衡论证"分层)
```

- ASCII 图用于结构关系(对象布局、阶段流水线、状态机)。代码块标注语言。
