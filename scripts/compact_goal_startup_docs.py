#!/usr/bin/env python3
"""Archive historical goal ledgers and install compact startup documents."""
...
PROTOCOL_PATH = ROOT / "docs" / "goal" / "goal-prompt.md"
STATE_PATH = ROOT / "docs" / "current-goal-state.md"
ARCHIVE_DIR = ROOT / "docs" / "archive" / "goal"
STATE_ARCHIVE = ARCHIVE_DIR / "current-goal-state-through-2026-07-09.md"


PROTOCOL = """# PCC goal execution protocol
...
def _archive(source: Path, archive: Path, minimum_size: int) -> None:
    if archive.exists():
        return
    if source.stat().st_size < minimum_size:
        raise RuntimeError(
            f"refusing to archive already-compacted or truncated source: {source}"
        )
...
def main() -> int:
    _archive(STATE_PATH, STATE_ARCHIVE, 1_900_000)

    board = load_task_board(TASK_BOARD)
    errors = validate(board)
    if errors:
        raise RuntimeError("invalid task board: " + "; ".join(errors))
    manifest = load_head_truth_manifest(HEAD_TRUTH_MANIFEST)

    PROTOCOL_PATH.write_text(PROTOCOL, encoding="utf-8")
    STATE_PATH.write_text(
        render_startup_markdown(board, manifest),
        encoding="utf-8",
    )
    print(f"archived historical ledgers under {ARCHIVE_DIR}")
