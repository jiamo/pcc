...
PyObject *py_int_to_bytes(PyObject *v, int64_t length, PyObject *byteorder) {
    ...
    if (PY_IS_TAGGED_INT(v)) {
        ...
        py_raise(py_exc_new(PY_EXC_TYPEERROR, "to_bytes expects an int"));
        return NULL;
    }
    ...
    if (needed > length) {
        py_raise(py_exc_new(PY_EXC_OVERFLOWERROR, "int too big to convert"));
        return NULL;
    }
...
PyObject *py_int_from_bytes(PyObject *bytes_obj, PyObject *byteorder) {
    ...
    int32_t tag = py_type_of(bytes_obj);
    ...
    PyBytesObject *b = (PyBytesObject *)bytes_obj;
    int64_t n = b->byte_len;
    ...
    for (int64_t i = 0; i < n; i++) {
        ...
        uint32_t byte = (uint32_t)(unsigned char)b->data[i];
