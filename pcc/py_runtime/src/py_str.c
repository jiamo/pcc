/* pcc/py_runtime/src/py_str.c
 *
 * Phase 2 UTF-8 string runtime for pcc's Python frontend.
 *
 * PyStrObject stores raw UTF-8 bytes inline (flexible array member) with
 * a NUL terminator one past the last byte, so both `const char*` access
 * and CPython-style length-prefixed access are O(1). Codepoint length
 * and the object hash are computed lazily and cached.
 *
 * Semantics:
 *   - Indexing and slicing are codepoint-aware: user-visible indices
 *     are codepoint indices, which we translate to byte offsets by
 *     scanning UTF-8 lead bytes (bytes where (b & 0xC0) != 0x80).
 *   - Equality, find, startswith, endswith, contains, split, replace
 *     all operate on bytes. For valid UTF-8 input this is equivalent to
 *     codepoint-level comparison.
 *   - `find` returns a codepoint offset (Python compatibility) while
 *     still scanning bytes internally.
 *   - upper/lower are ASCII-only in Phase 2. Full Unicode case mapping
 *     requires case-folding tables and is left as a TODO.
 *
 * All functions here (except private helpers) implement the ABI laid
 * out in pcc/py_runtime/include/py_runtime.h section 3.
 */

#include "py_internal.h"

#include <stdlib.h>
#include <string.h>
#include <assert.h>
#include <stdint.h>
#include <ctype.h>

/* ======================================================================
 * Internal helpers
 * ====================================================================== */

/* Allocate a new PyStrObject sized for `byte_len` payload bytes plus a
 * NUL terminator. Fields other than the data tail are initialised here
 * but the caller is responsible for copying bytes into ->data and for
 * writing the terminating NUL. Returns NULL on allocation failure.
 *
 * Safe to call with byte_len == 0; the returned string is the empty
 * string "" (still NUL terminated). */
static PyStrObject *py_str_alloc(int64_t byte_len) {
    if (byte_len < 0) return NULL;
    /* One extra byte for NUL. */
    size_t total = sizeof(PyStrObject) + (size_t)byte_len + 1u;
    PyStrObject *s = (PyStrObject *)malloc(total);
    if (s == NULL) return NULL;
    s->h.refcount = 1;
    s->h.type_tag = PY_TYPE_STR;
    s->h.flags    = 0;
    s->byte_len   = byte_len;
    s->cp_len     = -1;
    s->hash       = -1;
    s->data[byte_len] = '\0';   /* early NUL; caller overwrites payload */
    return s;
}

/* Count codepoints in a UTF-8 byte buffer.
 *
 * Uses the standard rule: a UTF-8 byte is a "lead" byte (start of a new
 * codepoint) iff (b & 0xC0) != 0x80. Equivalent to "number of non-
 * continuation bytes". No validation is performed — invalid UTF-8 is
 * counted leniently. */
static int64_t utf8_codepoint_count(const char *bytes, int64_t byte_len) {
    int64_t count = 0;
    for (int64_t i = 0; i < byte_len; i++) {
        unsigned char b = (unsigned char)bytes[i];
        if ((b & 0xC0) != 0x80) count++;
    }
    return count;
}

/* Return the byte offset of the `cp_idx`-th codepoint in `s`.
 *
 * Accepts:
 *   - 0 <= cp_idx <= cp_len(s): normal case; returns the starting byte
 *     of that codepoint. cp_idx == cp_len(s) returns byte_len (the
 *     "one past the end" offset, useful for slice bounds).
 *   - cp_idx < 0 or cp_idx > cp_len(s): returns byte_len (caller is
 *     responsible for clamping before calling for normal index ops). */
static int64_t utf8_byte_offset_for_codepoint(PyStrObject *s, int64_t cp_idx) {
    if (cp_idx <= 0) return 0;
    int64_t seen = 0;
    for (int64_t i = 0; i < s->byte_len; i++) {
        unsigned char b = (unsigned char)s->data[i];
        if ((b & 0xC0) != 0x80) {
            if (seen == cp_idx) return i;
            seen++;
        }
    }
    return s->byte_len;
}

/* Return the byte length of the UTF-8 codepoint whose lead byte is at
 * position `byte_off` in `s`. Returns 1 for an invalid lead byte, so
 * callers always make forward progress. */
static int64_t utf8_codepoint_byte_len(const PyStrObject *s, int64_t byte_off) {
    if (byte_off < 0 || byte_off >= s->byte_len) return 0;
    unsigned char b = (unsigned char)s->data[byte_off];
    if ((b & 0x80) == 0x00) return 1;                /* 0xxxxxxx       */
    if ((b & 0xE0) == 0xC0) return 2;                /* 110xxxxx 10... */
    if ((b & 0xF0) == 0xE0) return 3;                /* 1110xxxx 10... */
    if ((b & 0xF8) == 0xF0) return 4;                /* 11110xxx 10... */
    /* Malformed byte — advance by 1 so scanners still terminate. */
    return 1;
}

/* Normalise a codepoint index into [0, cp_len] for slice math.
 * Python slice semantics: negative indices wrap, out-of-range clamps. */
static int64_t clamp_slice_index(int64_t i, int64_t cp_len) {
    if (i < 0) {
        i += cp_len;
        if (i < 0) i = 0;
    } else if (i > cp_len) {
        i = cp_len;
    }
    return i;
}

/* Normalise a codepoint index into [-cp_len-1, cp_len-1] for str[i]
 * access — returns the resolved non-negative index, or -1 if out of
 * range. (Unlike slicing, plain indexing raises IndexError in CPython;
 * Phase 2 signals this with NULL and a TODO for py_raise.) */
static int64_t normalise_index(int64_t i, int64_t cp_len) {
    if (i < 0) i += cp_len;
    if (i < 0 || i >= cp_len) return -1;
    return i;
}

/* Compute (and cache) the codepoint length of `s`. */
static int64_t str_cp_len(PyStrObject *s) {
    if (s->cp_len < 0) {
        s->cp_len = utf8_codepoint_count(s->data, s->byte_len);
    }
    return s->cp_len;
}

/* Extract an int64 from a PyObject that's expected to be an int. On
 * NULL or None, returns `defval`. Tagged ints and heap bignums are both
 * supported via py_int_value_i64 (which aborts cleanly on non-int input
 * via its assert — callers must have already type-checked). For slice
 * math we don't expect bignum-sized indices, so truncation to int64 is
 * fine in practice. */
static int64_t int_or_default(PyObject *o, int64_t defval) {
    if (o == NULL || o == py_None) return defval;
    if (PY_IS_TAGGED_INT(o)) return py_untag_int(o);
    if (py_type_of(o) == PY_TYPE_INT) {
        return py_int_value_i64(o);
    }
    /* Unexpected type — fall back to default; caller handles semantics. */
    return defval;
}

/* ASCII whitespace predicate matching str.strip default set:
 *   space, tab, newline, carriage return, vertical tab, form feed. */
static int is_ascii_ws(unsigned char c) {
    return c == ' ' || c == '\t' || c == '\n'
        || c == '\r' || c == '\v' || c == '\f';
}

/* Search for needle in haystack; returns byte offset or -1 if missing.
 * Empty needle returns 0 (matches Python's `"abc".find("")` -> 0). */
static int64_t byte_find(const char *hay, int64_t hay_len,
                          const char *need, int64_t need_len) {
    if (need_len == 0) return 0;
    if (need_len > hay_len) return -1;
    /* Naive O(n*m) — good enough for Phase 2; revisit with two-way or
     * Boyer-Moore if profiling shows a hotspot. */
    int64_t last = hay_len - need_len;
    for (int64_t i = 0; i <= last; i++) {
        if (hay[i] == need[0] && memcmp(hay + i, need, (size_t)need_len) == 0) {
            return i;
        }
    }
    return -1;
}

/* Count codepoints in the prefix [0, byte_off) of `s`. Used to convert
 * a byte offset (what our byte-level search returns) into a codepoint
 * offset (what str.find should return for Python compat). */
static int64_t byte_offset_to_cp_offset(PyStrObject *s, int64_t byte_off) {
    if (byte_off <= 0) return 0;
    if (byte_off >= s->byte_len) byte_off = s->byte_len;
    return utf8_codepoint_count(s->data, byte_off);
}

/* Build a new PyStrObject from a contiguous UTF-8 byte range. */
static PyObject *str_from_range(const char *bytes, int64_t len) {
    PyStrObject *s = py_str_alloc(len);
    if (s == NULL) return NULL;
    if (len > 0) memcpy(s->data, bytes, (size_t)len);
    /* NUL written by py_str_alloc. */
    return (PyObject *)s;
}

/* ======================================================================
 * Construction & basic accessors
 * ====================================================================== */

PyObject *py_str_new(const char *utf8, int64_t byte_len) {
    if (byte_len < 0) byte_len = 0;
    /* NULL input with zero length is legal: gives back the empty str. */
    PyStrObject *s = py_str_alloc(byte_len);
    if (s == NULL) return NULL;
    if (utf8 != NULL && byte_len > 0) {
        memcpy(s->data, utf8, (size_t)byte_len);
    }
    /* cp_len and hash stay -1 (lazy) per contract. */
    return (PyObject *)s;
}

int64_t py_str_len(PyObject *s) {
    if (s == NULL) return 0;
    assert(!PY_IS_TAGGED_INT(s));
    assert(py_header(s)->type_tag == PY_TYPE_STR);
    return str_cp_len((PyStrObject *)s);
}

int64_t py_str_byte_len(PyObject *s) {
    if (s == NULL) return 0;
    return ((PyStrObject *)s)->byte_len;
}

const char *py_str_utf8(PyObject *s) {
    if (s == NULL) return "";
    return ((PyStrObject *)s)->data;
}

/* ======================================================================
 * Concatenation, repetition, slicing, indexing
 * ====================================================================== */

PyObject *py_str_concat(PyObject *a, PyObject *b) {
    if (a == NULL || b == NULL) return NULL;
    PyStrObject *sa = (PyStrObject *)a;
    PyStrObject *sb = (PyStrObject *)b;
    int64_t total = sa->byte_len + sb->byte_len;
    PyStrObject *out = py_str_alloc(total);
    if (out == NULL) return NULL;
    if (sa->byte_len > 0) memcpy(out->data, sa->data, (size_t)sa->byte_len);
    if (sb->byte_len > 0) memcpy(out->data + sa->byte_len, sb->data, (size_t)sb->byte_len);
    /* If both sides have their cp_len cached, we can add them cheaply. */
    if (sa->cp_len >= 0 && sb->cp_len >= 0) {
        out->cp_len = sa->cp_len + sb->cp_len;
    }
    return (PyObject *)out;
}

PyObject *py_str_repeat(PyObject *s, PyObject *n) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    int64_t count = int_or_default(n, 0);
    if (count <= 0 || ss->byte_len == 0) {
        /* Empty string result. */
        return py_str_new("", 0);
    }
    /* Overflow check: count * byte_len must fit in int64 and in size_t. */
    if (count > INT64_MAX / ss->byte_len) return NULL;
    int64_t total = count * ss->byte_len;
    PyStrObject *out = py_str_alloc(total);
    if (out == NULL) return NULL;
    for (int64_t i = 0; i < count; i++) {
        memcpy(out->data + i * ss->byte_len, ss->data, (size_t)ss->byte_len);
    }
    if (ss->cp_len >= 0) {
        out->cp_len = ss->cp_len * count;
    }
    return (PyObject *)out;
}

PyObject *py_str_slice(PyObject *s, PyObject *lo, PyObject *hi, PyObject *step) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    int64_t cp_len = str_cp_len(ss);

    /* Step: None -> 1; otherwise pulled out as int. Zero step = error;
     * Phase 2 returns NULL (TODO: raise ValueError). */
    int64_t step_v = (step == NULL || step == py_None) ? 1 : int_or_default(step, 1);
    if (step_v == 0) return NULL;

    /* Python slice default bounds depend on step sign:
     *   step > 0: lo defaults to 0, hi defaults to cp_len
     *   step < 0: lo defaults to cp_len-1, hi defaults to -cp_len-1
     *                                      (i.e. "before index 0") */
    int64_t lo_v, hi_v;
    if (step_v > 0) {
        lo_v = (lo == NULL || lo == py_None) ? 0      : int_or_default(lo, 0);
        hi_v = (hi == NULL || hi == py_None) ? cp_len : int_or_default(hi, cp_len);
        lo_v = clamp_slice_index(lo_v, cp_len);
        hi_v = clamp_slice_index(hi_v, cp_len);
        if (lo_v >= hi_v) return py_str_new("", 0);

        /* Fast path: step == 1 -> a contiguous byte-range copy. */
        if (step_v == 1) {
            int64_t bo_lo = utf8_byte_offset_for_codepoint(ss, lo_v);
            int64_t bo_hi = utf8_byte_offset_for_codepoint(ss, hi_v);
            return str_from_range(ss->data + bo_lo, bo_hi - bo_lo);
        }

        /* Step > 1: pick every step_v-th codepoint. We stream through
         * the string once, emitting selected codepoints into a buffer
         * sized for the worst case (the first selected codepoints are
         * the widest). */
        int64_t bo_lo = utf8_byte_offset_for_codepoint(ss, lo_v);
        int64_t bo_hi = utf8_byte_offset_for_codepoint(ss, hi_v);
        /* Upper bound: at most (bo_hi - bo_lo) bytes. */
        int64_t cap = bo_hi - bo_lo;
        PyStrObject *out = py_str_alloc(cap);
        if (out == NULL) return NULL;
        int64_t out_bytes = 0;
        int64_t out_cps   = 0;
        int64_t cp_index  = lo_v;
        int64_t b = bo_lo;
        int64_t next_target = lo_v;
        while (b < bo_hi) {
            int64_t w = utf8_codepoint_byte_len(ss, b);
            if (cp_index == next_target) {
                memcpy(out->data + out_bytes, ss->data + b, (size_t)w);
                out_bytes += w;
                out_cps++;
                next_target += step_v;
            }
            b += w;
            cp_index++;
        }
        out->byte_len = out_bytes;
        out->data[out_bytes] = '\0';
        out->cp_len = out_cps;
        return (PyObject *)out;
    }

    /* step_v < 0 (reverse walk).
     *
     * Python: s[lo:hi:step] with negative step walks downward from lo
     * (default cp_len - 1) toward hi (default "before 0"), stopping
     * *strictly* above hi. */
    int64_t default_lo = cp_len - 1;
    int64_t default_hi = -1;   /* one before index 0 */
    lo_v = (lo == NULL || lo == py_None) ? default_lo : int_or_default(lo, default_lo);
    hi_v = (hi == NULL || hi == py_None) ? default_hi : int_or_default(hi, default_hi);

    /* Resolve negative lo/hi into normal space but allow lo to sit at
     * cp_len (clamped) and hi to sit at -1 (meaning "before 0"). */
    if (lo_v < 0) lo_v += cp_len;
    if (lo_v >= cp_len) lo_v = cp_len - 1;
    if (lo_v < 0) return py_str_new("", 0);

    if (hi != NULL && hi != py_None) {
        if (hi_v < 0) hi_v += cp_len;
        /* Clamp hi to [-1, cp_len]. */
        if (hi_v < -1) hi_v = -1;
        if (hi_v > cp_len) hi_v = cp_len;
    }

    if (lo_v <= hi_v) return py_str_new("", 0);

    /* Count how many codepoints we'll emit: from lo_v down to (hi_v + 1)
     * inclusive, stepping by |step_v|. */
    int64_t span = lo_v - hi_v;        /* positive */
    int64_t pos_step = -step_v;
    int64_t out_n = (span + pos_step - 1) / pos_step;  /* ceiling div */

    /* Build an index of byte offsets keyed by codepoint index so we can
     * jump around without rescanning for each pick. */
    int64_t *cp_off = (int64_t *)malloc(sizeof(int64_t) * (size_t)(cp_len + 1));
    if (cp_off == NULL) return NULL;
    {
        int64_t cp = 0;
        for (int64_t i = 0; i < ss->byte_len; i++) {
            unsigned char b = (unsigned char)ss->data[i];
            if ((b & 0xC0) != 0x80) {
                cp_off[cp++] = i;
            }
        }
        cp_off[cp_len] = ss->byte_len;
    }

    /* Worst-case buffer size = byte_len of source. */
    PyStrObject *out = py_str_alloc(ss->byte_len);
    if (out == NULL) { free(cp_off); return NULL; }
    int64_t out_bytes = 0;
    for (int64_t k = 0; k < out_n; k++) {
        int64_t cp = lo_v + step_v * k;   /* step_v is negative */
        int64_t start = cp_off[cp];
        int64_t end   = cp_off[cp + 1];
        int64_t w = end - start;
        memcpy(out->data + out_bytes, ss->data + start, (size_t)w);
        out_bytes += w;
    }
    out->byte_len = out_bytes;
    out->data[out_bytes] = '\0';
    out->cp_len = out_n;
    free(cp_off);
    return (PyObject *)out;
}

PyObject *py_str_index(PyObject *s, PyObject *i) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    int64_t idx = int_or_default(i, 0);
    int64_t cp_len = str_cp_len(ss);
    int64_t real = normalise_index(idx, cp_len);
    if (real < 0) {
        /* TODO(phase3): py_raise IndexError. */
        return NULL;
    }
    int64_t bo = utf8_byte_offset_for_codepoint(ss, real);
    int64_t w  = utf8_codepoint_byte_len(ss, bo);
    PyObject *out = str_from_range(ss->data + bo, w);
    if (out != NULL) {
        /* Single-codepoint string: cache cp_len = 1. */
        ((PyStrObject *)out)->cp_len = 1;
    }
    return out;
}

/* ======================================================================
 * Comparisons & lookup
 * ====================================================================== */

int py_str_eq(PyObject *a, PyObject *b) {
    if (a == b) return 1;
    if (a == NULL || b == NULL) return 0;
    PyStrObject *sa = (PyStrObject *)a;
    PyStrObject *sb = (PyStrObject *)b;
    if (sa->byte_len != sb->byte_len) return 0;
    if (sa->byte_len == 0) return 1;
    return memcmp(sa->data, sb->data, (size_t)sa->byte_len) == 0;
}

int py_str_contains(PyObject *s, PyObject *sub) {
    if (s == NULL || sub == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *sn = (PyStrObject *)sub;
    return byte_find(ss->data, ss->byte_len, sn->data, sn->byte_len) != -1;
}

int64_t py_str_find(PyObject *s, PyObject *sub) {
    if (s == NULL || sub == NULL) return -1;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *sn = (PyStrObject *)sub;
    int64_t bo = byte_find(ss->data, ss->byte_len, sn->data, sn->byte_len);
    if (bo < 0) return -1;
    /* Contract: return codepoint offset for Python compatibility. */
    return byte_offset_to_cp_offset(ss, bo);
}

int py_str_startswith(PyObject *s, PyObject *prefix) {
    if (s == NULL || prefix == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *sp = (PyStrObject *)prefix;
    if (sp->byte_len > ss->byte_len) return 0;
    if (sp->byte_len == 0) return 1;
    return memcmp(ss->data, sp->data, (size_t)sp->byte_len) == 0;
}

int py_str_endswith(PyObject *s, PyObject *suffix) {
    if (s == NULL || suffix == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *sf = (PyStrObject *)suffix;
    if (sf->byte_len > ss->byte_len) return 0;
    if (sf->byte_len == 0) return 1;
    return memcmp(ss->data + (ss->byte_len - sf->byte_len),
                  sf->data, (size_t)sf->byte_len) == 0;
}

/* ======================================================================
 * ASCII case mapping (Phase 2)
 * ====================================================================== */
/* Full Unicode case mapping requires case-folding tables (e.g. from
 * UCD's CaseFolding.txt). Phase 2 implements ASCII-only conversion:
 * bytes 0x41..0x5A flip with 0x61..0x7A; all other bytes pass through.
 * This preserves byte_len (one byte changes to another byte, never
 * widening) which keeps the cp_len count valid. */

PyObject *py_str_upper(PyObject *s) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *out = py_str_alloc(ss->byte_len);
    if (out == NULL) return NULL;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        unsigned char c = (unsigned char)ss->data[i];
        if (c >= 'a' && c <= 'z') c = (unsigned char)(c - ('a' - 'A'));
        out->data[i] = (char)c;
    }
    /* TODO(phase3): full Unicode case mapping for non-ASCII codepoints. */
    out->cp_len = ss->cp_len;   /* unchanged by ASCII case flip */
    return (PyObject *)out;
}

PyObject *py_str_lower(PyObject *s) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *out = py_str_alloc(ss->byte_len);
    if (out == NULL) return NULL;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        unsigned char c = (unsigned char)ss->data[i];
        if (c >= 'A' && c <= 'Z') c = (unsigned char)(c + ('a' - 'A'));
        out->data[i] = (char)c;
    }
    /* TODO(phase3): full Unicode case mapping for non-ASCII codepoints. */
    out->cp_len = ss->cp_len;
    return (PyObject *)out;
}

/* ======================================================================
 * Whitespace stripping
 * ====================================================================== */

PyObject *py_str_strip(PyObject *s) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    int64_t lo = 0;
    int64_t hi = ss->byte_len;
    while (lo < hi && is_ascii_ws((unsigned char)ss->data[lo])) lo++;
    while (hi > lo && is_ascii_ws((unsigned char)ss->data[hi - 1])) hi--;
    return str_from_range(ss->data + lo, hi - lo);
}

PyObject *py_str_lstrip(PyObject *s) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    int64_t lo = 0;
    int64_t hi = ss->byte_len;
    while (lo < hi && is_ascii_ws((unsigned char)ss->data[lo])) lo++;
    return str_from_range(ss->data + lo, hi - lo);
}

PyObject *py_str_rstrip(PyObject *s) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    int64_t lo = 0;
    int64_t hi = ss->byte_len;
    while (hi > lo && is_ascii_ws((unsigned char)ss->data[hi - 1])) hi--;
    return str_from_range(ss->data + lo, hi - lo);
}

/* chars-aware variants. Strip / lstrip / rstrip on a string using
 * an explicit set of characters (given as a PyStrObject). */
static int is_in_chars(unsigned char c, const unsigned char *chars, int64_t n) {
    for (int64_t k = 0; k < n; k++) {
        if (chars[k] == c) return 1;
    }
    return 0;
}

PyObject *py_str_strip_chars(PyObject *s, PyObject *chars) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *cs = (PyStrObject *)chars;
    int64_t lo = 0;
    int64_t hi = ss->byte_len;
    while (lo < hi && is_in_chars(
        (unsigned char)ss->data[lo],
        (const unsigned char *)cs->data, cs->byte_len)) lo++;
    while (hi > lo && is_in_chars(
        (unsigned char)ss->data[hi - 1],
        (const unsigned char *)cs->data, cs->byte_len)) hi--;
    return str_from_range(ss->data + lo, hi - lo);
}

PyObject *py_str_lstrip_chars(PyObject *s, PyObject *chars) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *cs = (PyStrObject *)chars;
    int64_t lo = 0;
    int64_t hi = ss->byte_len;
    while (lo < hi && is_in_chars(
        (unsigned char)ss->data[lo],
        (const unsigned char *)cs->data, cs->byte_len)) lo++;
    return str_from_range(ss->data + lo, hi - lo);
}

PyObject *py_str_rstrip_chars(PyObject *s, PyObject *chars) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *cs = (PyStrObject *)chars;
    int64_t lo = 0;
    int64_t hi = ss->byte_len;
    while (hi > lo && is_in_chars(
        (unsigned char)ss->data[hi - 1],
        (const unsigned char *)cs->data, cs->byte_len)) hi--;
    return str_from_range(ss->data + lo, hi - lo);
}

/* ASCII-only predicates — pcc doesn't yet model the full Unicode
 * property tables, but every pcc source that uses these checks
 * them against source-code-style tokens which are ASCII. */
int py_str_isdigit(PyObject *s) {
    if (s == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    if (ss->byte_len == 0) return 0;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        unsigned char c = (unsigned char)ss->data[i];
        if (c < '0' || c > '9') return 0;
    }
    return 1;
}

int py_str_isalpha(PyObject *s) {
    if (s == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    if (ss->byte_len == 0) return 0;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        unsigned char c = (unsigned char)ss->data[i];
        int ok = (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z');
        if (!ok) return 0;
    }
    return 1;
}

int py_str_isspace(PyObject *s) {
    if (s == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    if (ss->byte_len == 0) return 0;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        if (!is_ascii_ws((unsigned char)ss->data[i])) return 0;
    }
    return 1;
}

int py_str_isalnum(PyObject *s) {
    if (s == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    if (ss->byte_len == 0) return 0;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        unsigned char c = (unsigned char)ss->data[i];
        int ok = (c >= '0' && c <= '9')
                 || (c >= 'a' && c <= 'z')
                 || (c >= 'A' && c <= 'Z');
        if (!ok) return 0;
    }
    return 1;
}

/* ``s.count(sub)`` — returns the number of non-overlapping
 * occurrences of ``sub`` in ``s``. */
int64_t py_str_count(PyObject *s, PyObject *sub) {
    if (s == NULL || sub == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *ps = (PyStrObject *)sub;
    if (ps->byte_len == 0) return ss->byte_len + 1;
    int64_t count = 0;
    int64_t i = 0;
    while (i + ps->byte_len <= ss->byte_len) {
        if (ss->data[i] == ps->data[0]
            && memcmp(ss->data + i, ps->data, (size_t)ps->byte_len) == 0) {
            count++;
            i += ps->byte_len;
        } else {
            i++;
        }
    }
    return count;
}

/* ======================================================================
 * split / join / replace
 * ====================================================================== */

/* Split on runs of ASCII whitespace (Python's `str.split()` default).
 * Leading and trailing runs are ignored; each result item is non-empty. */
static PyObject *py_str_split_whitespace(PyStrObject *ss) {
    PyObject *list = py_list_new(4);
    if (list == NULL) return NULL;
    int64_t i = 0;
    while (i < ss->byte_len) {
        while (i < ss->byte_len && is_ascii_ws((unsigned char)ss->data[i])) i++;
        if (i >= ss->byte_len) break;
        int64_t start = i;
        while (i < ss->byte_len && !is_ascii_ws((unsigned char)ss->data[i])) i++;
        PyObject *part = str_from_range(ss->data + start, i - start);
        if (part == NULL) { py_decref(list); return NULL; }
        py_list_append(list, part);
        py_decref(part);   /* list holds its own ref via append */
    }
    return list;
}

PyObject *py_str_split(PyObject *s, PyObject *sep) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;

    /* Default: split on whitespace runs, no empty strings in result. */
    if (sep == NULL || sep == py_None) {
        return py_str_split_whitespace(ss);
    }

    PyStrObject *sp = (PyStrObject *)sep;
    if (sp->byte_len == 0) {
        /* Python raises ValueError("empty separator"). Phase 2: return
         * an empty list to avoid infinite-loop UB; TODO: py_raise. */
        return py_list_new(0);
    }

    PyObject *list = py_list_new(4);
    if (list == NULL) return NULL;

    int64_t start = 0;
    int64_t i     = 0;
    while (i + sp->byte_len <= ss->byte_len) {
        if (ss->data[i] == sp->data[0]
            && memcmp(ss->data + i, sp->data, (size_t)sp->byte_len) == 0)
        {
            PyObject *part = str_from_range(ss->data + start, i - start);
            if (part == NULL) { py_decref(list); return NULL; }
            py_list_append(list, part);
            py_decref(part);
            i += sp->byte_len;
            start = i;
        } else {
            i++;
        }
    }
    /* Final trailing segment (possibly empty, which matches CPython). */
    PyObject *tail = str_from_range(ss->data + start, ss->byte_len - start);
    if (tail == NULL) { py_decref(list); return NULL; }
    py_list_append(list, tail);
    py_decref(tail);
    return list;
}

PyObject *py_str_join(PyObject *sep, PyObject *list) {
    if (sep == NULL || list == NULL) return NULL;
    PyStrObject *sp = (PyStrObject *)sep;
    PyListObject *l = (PyListObject *)list;

    if (l->length == 0) return py_str_new("", 0);

    /* First pass: validate types and compute total byte length. Python
     * raises TypeError when a list element isn't a str; Phase 2 returns
     * NULL — TODO: py_raise. */
    int64_t total = 0;
    for (int64_t i = 0; i < l->length; i++) {
        PyObject *e = l->items[i];
        if (e == NULL || py_type_of(e) != PY_TYPE_STR) return NULL;
        if (i > 0) total += sp->byte_len;
        total += ((PyStrObject *)e)->byte_len;
    }

    PyStrObject *out = py_str_alloc(total);
    if (out == NULL) return NULL;

    int64_t off = 0;
    for (int64_t i = 0; i < l->length; i++) {
        PyStrObject *e = (PyStrObject *)l->items[i];
        if (i > 0 && sp->byte_len > 0) {
            memcpy(out->data + off, sp->data, (size_t)sp->byte_len);
            off += sp->byte_len;
        }
        if (e->byte_len > 0) {
            memcpy(out->data + off, e->data, (size_t)e->byte_len);
            off += e->byte_len;
        }
    }
    return (PyObject *)out;
}

PyObject *py_str_replace(PyObject *s, PyObject *old, PyObject *new_) {
    if (s == NULL || old == NULL || new_ == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyStrObject *so = (PyStrObject *)old;
    PyStrObject *sn = (PyStrObject *)new_;

    /* Empty `old` in CPython inserts `new_` between every codepoint and
     * at both ends. For Phase 2 we keep the simple behaviour: return a
     * copy of `s` (matches many other runtimes). TODO: full semantics. */
    if (so->byte_len == 0) {
        return py_str_new(ss->data, ss->byte_len);
    }

    /* First pass: count matches to size output exactly. */
    int64_t matches = 0;
    {
        int64_t i = 0;
        while (i + so->byte_len <= ss->byte_len) {
            if (ss->data[i] == so->data[0]
                && memcmp(ss->data + i, so->data, (size_t)so->byte_len) == 0)
            {
                matches++;
                i += so->byte_len;
            } else {
                i++;
            }
        }
    }
    if (matches == 0) {
        /* Nothing to do — still allocate a fresh string so callers
         * receive a new reference (matches Python semantics). */
        return py_str_new(ss->data, ss->byte_len);
    }

    int64_t delta = (sn->byte_len - so->byte_len) * matches;
    /* Guard against pathological overflow. */
    int64_t total = ss->byte_len + delta;
    if (total < 0) return NULL;

    PyStrObject *out = py_str_alloc(total);
    if (out == NULL) return NULL;

    int64_t read  = 0;
    int64_t write = 0;
    while (read + so->byte_len <= ss->byte_len) {
        if (ss->data[read] == so->data[0]
            && memcmp(ss->data + read, so->data, (size_t)so->byte_len) == 0)
        {
            if (sn->byte_len > 0) {
                memcpy(out->data + write, sn->data, (size_t)sn->byte_len);
                write += sn->byte_len;
            }
            read += so->byte_len;
        } else {
            out->data[write++] = ss->data[read++];
        }
    }
    /* Copy tail (after last possible match position). */
    while (read < ss->byte_len) {
        out->data[write++] = ss->data[read++];
    }
    /* byte_len/NUL were set by py_str_alloc; invariants hold as long as
     * our first-pass count matched the second-pass loop — which it does
     * by construction. */
    return (PyObject *)out;
}

/* ======================================================================
 * Hashing (FNV-1a) — exposed via py_obj_hash in a later patch; kept
 * here so we own the cache slot on PyStrObject.
 * ====================================================================== */

/* Public helper — used by py_obj_hash once P2 wires dict up. FNV-1a on
 * the raw UTF-8 bytes; cached on the object. Never returns -1 (which
 * the field uses as the "not computed" sentinel); maps -1 to -2. */
/* Internal splitlines — keepends controls whether the trailing
 * line-ending bytes stay in each fragment. */
static PyObject *py_str_splitlines_impl(PyObject *s, int keepends) {
    if (s == NULL) return NULL;
    PyStrObject *ss = (PyStrObject *)s;
    PyObject *list = py_list_new(4);
    if (list == NULL) return NULL;
    int64_t start = 0;
    int64_t i = 0;
    while (i < ss->byte_len) {
        unsigned char c = (unsigned char)ss->data[i];
        if (c == '\r' || c == '\n') {
            int64_t end = i;
            int64_t after;
            if (c == '\r' && i + 1 < ss->byte_len && ss->data[i + 1] == '\n') {
                after = i + 2;
            } else {
                after = i + 1;
            }
            int64_t frag_end = keepends ? after : end;
            PyObject *part = str_from_range(
                ss->data + start, frag_end - start);
            if (part == NULL) { py_decref(list); return NULL; }
            py_list_append(list, part);
            py_decref(part);
            i = after;
            start = after;
        } else {
            i += 1;
        }
    }
    if (start < ss->byte_len) {
        PyObject *tail = str_from_range(
            ss->data + start, ss->byte_len - start);
        if (tail == NULL) { py_decref(list); return NULL; }
        py_list_append(list, tail);
        py_decref(tail);
    }
    return list;
}

PyObject *py_str_splitlines_keepends(PyObject *s, int keepends) {
    return py_str_splitlines_impl(s, keepends ? 1 : 0);
}

/* ``s.splitlines()`` — split on any of \n, \r, \r\n; separator bytes
 * are *not* included in fragments. */
PyObject *py_str_splitlines(PyObject *s) {
    return py_str_splitlines_impl(s, 0);
}

int64_t py_str_hash(PyObject *s) {
    if (s == NULL) return 0;
    PyStrObject *ss = (PyStrObject *)s;
    if (ss->hash != -1) return ss->hash;

    /* FNV-1a 64-bit. */
    uint64_t h = 0xcbf29ce484222325ull;
    const uint64_t prime = 0x100000001b3ull;
    for (int64_t i = 0; i < ss->byte_len; i++) {
        h ^= (unsigned char)ss->data[i];
        h *= prime;
    }

    int64_t result = (int64_t)h;
    if (result == -1) result = -2;   /* avoid sentinel collision */
    ss->hash = result;
    return result;
}
