...
pcc_dealloc_with_trash = extern(
    "pcc_dealloc_with_trash", (c_ptr, c_int64), c_void,
)
...
pcc_gc_note_object_allocated = extern(
    "pcc_gc_note_object_allocated", (c_ptr,), c_void,
)
...
pcc_gc_backend4_try_zpage_alloc = extern(
    "pcc_gc_backend4_try_zpage_alloc", (c_int64, c_int32), c_ptr,
)
...
pcc_gc_note_frame_enter = extern(
    "pcc_gc_note_frame_enter", (c_ptr, c_ptr), c_void,
)
pcc_gc_note_frame_leave = extern(
    "pcc_gc_note_frame_leave", (c_ptr,), c_void,
)
...
pcc_gc_begin_explicit_tracing_collect = extern(
    "pcc_gc_begin_explicit_tracing_collect", (), c_void,
)
pcc_gc_end_explicit_tracing_collect = extern(
    "pcc_gc_end_explicit_tracing_collect", (), c_void,
)
...
pcc_debug_note_alloc_size = extern("pcc_debug_note_alloc_size", (c_ptr, c_int64), c_void)
...
def pcc_gc_load_ptr(owner, slot):
    ...
    if backend == 3 or backend == 4:
        pcc_gc_note_load()
        v = load_ptr(slot, 0)
        if ptr_is_null(v) == 0 and is_tagged_int(v) == 0:
...
def _pcc_gc_callback_eq(a, b) -> int:
    ...
    if load_i32(a, 8) == 9:
        if load_i32(b, 8) == 9:
            if ptr_eq(load_ptr(a, 16), load_ptr(b, 16)) != 0:
                a_captures = pcc_gc_load_ptr(a, ptr_add(a, 24))
                b_captures = pcc_gc_load_ptr(b, ptr_add(b, 24))
...
def _ptr_can_have_header(o) -> bool:
    ...
    if (bits & 3) != 0:
        return False
    if bits >= 140737488355328:  # 1 << 47, because ``bits`` is ptr >> 1.
        return False
    return True
...
def py_incref(o) -> None:
    ...
    if tag < 0 or (tag > 31 and tag < 100) or tag > 500:
        return
    flags: int = load_i32(o, 12)
    if (
...
def py_decref(o) -> None:
    ...
    if tag_dbg < 0 or (tag_dbg > 31 and tag_dbg < 100) or tag_dbg > 500:
        return
    flags: int = load_i32(o, 12)
    if (
