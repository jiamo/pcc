...
/* pcc local patch: the upstream macro uses a C99 compound literal
 * (((union {...}){x}).i), which pcc's C parser does not accept yet
 * (BUG-P1-CC-COMPOUND-LITERAL-UNSUPPORTED). A static inline with a
 * named union is the same bit reinterpretation. */
static inline uint64_t pcc_asuint64(double v) {
	union { double f; uint64_t i; } u;
	u.f = v;
	return u.i;
}
#define ASUINT64(x) pcc_asuint64(x)
...
{
	/* pcc local patch: #pragma STDC FENV_ACCESS inside a function body is
	 * not accepted by pcc's parser; the algorithm does not depend on
	 * dynamic rounding-mode access here. */
