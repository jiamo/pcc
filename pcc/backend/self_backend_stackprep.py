...
from .self_backend_ir import AllocaInfo, I1, ParsedFunction, SlotInfo, TypeDesc, _align_to
    ...
    *,
    ...
    def alloc_value_slot(value_name: str, value_type: TypeDesc) -> SlotInfo:
        ...
        if value_name not in last_uses:
            return SlotInfo(alloc(value_type.value_slot_size, value_type.value_align), value_type)
    ...
    for arg in func.args:
        if arg.type.is_void or arg.name not in func.used_values:
            continue
    ...
    if aggregate_returned_indirect(func.ret_type):
        func.hidden_sret_slot = SlotInfo(alloc(8, 8), TypeDesc("ptr", pointee=TypeDesc("void")))

    for block in func.blocks:
        current_block = block.name
        for phi in block.phis:
            func.value_types[phi.dest] = phi.type
            if phi.dest in func.used_values:
                func.value_slots.setdefault(phi.dest, alloc_value_slot(phi.dest, phi.type))
        for index, instr in enumerate(block.instructions):
            ...
            if kind == "alloca":
                ...
                if dest in func.used_values:
                    func.value_slots.setdefault(dest, alloc_value_slot(dest, value_type))
            elif kind == "binop":
                ...
                if dest in func.used_values:
                    func.value_slots.setdefault(dest, alloc_value_slot(dest, result_type))
            elif kind == "fcmp":
                _cond, dest, _value_type, _lhs, _rhs = data
                result_type = (
                    TypeDesc("array", count=_value_type.count, elem=I1)
                ...
                dest, vector_type, _vector_value, _elem_type, _elem_value, _index_value = data
                func.value_types[dest] = vector_type
                if dest in func.used_values:
                    func.value_slots.setdefault(dest, alloc_value_slot(dest, vector_type))
                ...
                dest, aggregate_type, _aggregate_value, _elem_type, _elem_value, _indices, _offset = data
                func.value_types[dest] = aggregate_type
                if dest in func.used_values:
                    func.value_slots.setdefault(dest, alloc_value_slot(dest, aggregate_type))
            elif kind == "va_arg":
                ...
                dest, ret_type, _callee, _is_indirect, _args, _fixed_arg_count, _is_vararg_call = data
                if dest is not None:
                    func.value_types[dest] = ret_type
                    if dest in func.used_values:
                        func.value_slots.setdefault(dest, alloc_value_slot(dest, ret_type))
