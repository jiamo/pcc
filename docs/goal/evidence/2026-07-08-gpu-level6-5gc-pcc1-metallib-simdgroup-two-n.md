# 2026-07-08 five-GC pcc1 Metallib Simdgroup Two-N Evidence
...
simdgroup/tensorcore-style prebuilt `.metallib` workload beyond the initial
8x8 microkernel.

The covered workload is a two-N 8x16 output GEMM with f16 A/B inputs and f32 C
output:
...
`simdgroup_index_in_threadgroup`, split the simdgroup tile over N with
`uint simdgroup_tile_n = simdgroup_idx % 2u;`, and store with the expected
N-stride `simdgroup_store(..., 16u, ...)` before compiling
`.metal -> .air -> .metallib`.
...
Host metallib two-N simdgroup probe result:
`metal_artifacts_produced`, `metallib True`, bridge load-validated,
`simdgroup_multiply_accumulate True`, `simdgroup_tile_n True`, and N-stride
store proof true.
...
gtimeout 180s env -u LC_ALL \
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_level6_classifier_accepts_complete_pcc1_five_gc_simdgroup_two_n_metallib_matrix \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_two_n_metallib_lifetime_real_or_skipped \
  -rs
```
...
workload: the first two-N simdgroup GEMM, covering two 8x8 simdgroup tiles
across the N axis inside one 8x16 output. It does not prove broader
simdgroup/tensorcore metallib tiling across M, 2D grids, split-K atomics,
edge/tail guards, more-than-two simdgroups, non-f32 atomics, arbitrary
