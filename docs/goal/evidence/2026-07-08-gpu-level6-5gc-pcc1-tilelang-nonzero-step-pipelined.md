# 2026-07-08 GPU Level-6 Five-GC pcc1 TileLang Nonzero T.Pipelined Step Evidence

## Summary

`GPU-P1-BROADER-TILELANG-TIRX-PASSES` now has Level-6 five-GC proof for the
same pcc1-native nonzero-start plus stepped `T.Pipelined(start, end, step, ...)`
scalar tiled GEMM shape covered by the Level-5 slice.

The test builds one pcc1 no-libpython executable for the runtime-source
TileLang nonzero stepped-pipelined package, then runs that same executable with
...
gtimeout 1500s env -u LC_ALL \
  ...
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_tilelang_nonzero_step_pipelined_lifetime_real_or_skipped -rs
```

Result: `1 passed in 2.37s`.
