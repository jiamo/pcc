# 2026-07-08 GPU Level-6 Five-GC pcc1 Simdgroup Sixteen Transpose_AB Split-K Atomic Edge/Tail Evidence

## Summary

`GPU-P0-SIMDGROUP-TENSORCORE-GEMM` now has Level-6 five-GC proof for the first
sixteen-simdgroup opt-in Metal simdgroup GEMM tile that combines `transpose_A`,
`transpose_B`, split-K atomic output, M/N edge tiles, and a K tail:
`M=31,N=31,K=17`, `block_M=32,block_N=32,block_K=8`, `split_k=4`,
`split_k_span_mode=ceildiv`, and `threads=512`.

The test builds one pcc1 no-libpython executable for the sixteen-simdgroup
...
creates real native MTLBuffers, writes A(K,M)=A(17,31) and B(N,K)=B(31,17) f16
...
synchronous fence-completed command buffer, reads exact f32 C(31,31) output,
...
gtimeout 1200s env -u LC_ALL \
  ...
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_sixteen_transpose_ab_splitk_atomic_edge_tail_lifetime_real_or_skipped -rs
```

Result: `1 passed in 5.26s`.

```bash
gtimeout 1320s env -u LC_ALL \
  PCC_CURRENT_PCC1=build/bootstrap-gpu-level5-pcc1-shim/pcc1 \
  PCC_RUN_GPU_5GC_LIFETIME=1 \
  uv run pytest -q -n0 tests/gpu_hardware/test_metal_5gc_lifetime_real.py -rs
```

Result: `25 passed in 44.87s`.
...
This proves `GPU_LEVEL_6_5GC_PARITY` for one sixteen-simdgroup opt-in simdgroup
