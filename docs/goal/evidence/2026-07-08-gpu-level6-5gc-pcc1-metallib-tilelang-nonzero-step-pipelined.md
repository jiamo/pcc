# 2026-07-08 five-GC pcc1 Metallib TileLang Nonzero Stepped Pipelined Evidence
...
imported TileLang/TIRx nonzero-start stepped `T.Pipelined(...)` scalar GEMM
...
Host metallib nonzero stepped Pipelined probe result:
...
gtimeout 180s env -u LC_ALL \
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_level6_classifier_accepts_complete_pcc1_five_gc_tilelang_nonzero_step_pipelined_metallib_matrix \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_tilelang_nonzero_step_pipelined_metallib_lifetime_real_or_skipped \
  -rs
```
...
`T.Pipelined(...)` scalar GEMM with the static K-loop
`for (uint ko = 1u; ko < 5u; ko += 2u)`. It does not prove arbitrary nested
or dynamic loop forms, arbitrary stepped `T.Pipelined` expressions beyond this
static shape, executable vectorized loop bodies beyond the covered copy
staging, broader TileLang/TIRx variants, broader simdgroup/tensorcore variants,
