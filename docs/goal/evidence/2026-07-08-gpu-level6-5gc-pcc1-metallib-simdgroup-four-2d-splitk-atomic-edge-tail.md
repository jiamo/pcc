# 2026-07-08 five-GC pcc1 Metallib Simdgroup Four-2D Split-K Atomic Edge/Tail Evidence
...
simdgroup/tensorcore-style prebuilt `.metallib` workload combining split-K
atomic accumulation with non-divisible M/N/K edge-tail guards.
...
stage guarded A/B shared tiles, and accumulate guarded output with:
...
f16 A/B payloads with byte-exact stores for the odd payload sizes,
zero-initializes the f32 C payload before launch, calls
`pcc_metal_metallib_runtime_call_prebuilt(...)` with the produced `.metallib`,
waits for fence completion, reads f32 C back as exact bit patterns, checks the
`execute_scalar_tiled_gemm_reference(...)` CPU oracle, and releases native
buffers only after readback/fence-completed launch.
...
gtimeout 180s env -u LC_ALL \
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_level6_classifier_accepts_complete_pcc1_five_gc_simdgroup_four_2d_splitk_atomic_edge_tail_metallib_matrix \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_four_2d_splitk_atomic_edge_tail_metallib_lifetime_real_or_skipped \
  -rs
```
...
workload: the first four-simdgroup 2D split-K atomic edge/tail GEMM, covering
four z-axis split-K launches over a 15x15 output and K=17 reduction with
ceildiv-clamped K ranges, guarded A/B zero-fill, explicit C zero
initialization, and f32 atomic accumulation. It does not prove transpose_AB
split-K atomic metallib, more-than-four simdgroups, non-f32 atomics, arbitrary
TileLang/TIRx lowering, external framework DLPack/stream interop, deployment
packaging UX, performance, or whole-program GPU execution.
