# 2026-07-07 GPU Level-6 Five-GC pcc1 TileLang Nonzero Pipelined Evidence
...
The test builds one pcc1 no-libpython executable for the nonzero-start
Pipelined runtime-source package, then runs that same executable with
...
reads exact f32 output, and releases native buffers after the fence.
...
gtimeout 900s env -u LC_ALL \
  ...
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_tilelang_nonzero_start_pipelined_lifetime_real_or_skipped -rs
```

Result: `1 passed in 2.68s`.
