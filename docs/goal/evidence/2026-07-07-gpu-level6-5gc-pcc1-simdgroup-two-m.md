# 2026-07-07 GPU Level-6 Five-GC pcc1 Simdgroup Two-M Evidence

## Summary

`GPU-P0-SIMDGROUP-TENSORCORE-GEMM` now has Level-6 five-GC proof for the two-M
opt-in Metal simdgroup GEMM tile: `M=16,N=8,K=8` with
`block_M=16,block_N=8,block_K=8` and `threads=64`.

The test builds one pcc1 no-libpython executable for the two-M simdgroup
...
gtimeout 900s env -u LC_ALL \
  ...
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_two_m_lifetime_real_or_skipped -rs
```
