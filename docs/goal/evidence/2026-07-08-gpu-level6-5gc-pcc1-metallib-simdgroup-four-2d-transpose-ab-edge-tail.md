# 2026-07-08 five-GC pcc1 Metallib Simdgroup Four-2D Transpose_AB Edge/Tail Evidence
...
simdgroup/tensorcore-style prebuilt `.metallib` workload combining 2D MxN
tiling, transpose_A+transpose_B operand layout, and non-divisible M/N/K
edge-tail guards.
...
```text
A: 9x15 f16, read as transposed A
B: 15x9 f16, read as transposed B
C: 15x15 f32
...
axes, zero-fill guarded K-tail loads, use the transposed A/B index forms, and
guard final C writeback:

```text
A_tile[...] = (global_m < 15u && global_k < 9u) ? A[(global_k * 15u) + global_m] : half(0.0)
B_tile[...] = (global_k < 9u && global_n < 15u) ? B[(global_n * 9u) + global_k] : half(0.0)
if (row < 15u && col < 15u) { ... }
```
...
gtimeout 180s env -u LC_ALL \
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_level6_classifier_accepts_complete_pcc1_five_gc_simdgroup_four_2d_transpose_ab_edge_tail_metallib_matrix \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_four_2d_transpose_ab_edge_tail_metallib_lifetime_real_or_skipped \
  -rs
```
...
workload: the first four-simdgroup 2D transpose_A+transpose_B edge/tail GEMM,
covering a 2x2 grid of 8x8 simdgroup tiles over a 15x15 output and K=9
reduction with guarded transposed zero-fill loads and guarded C writeback. It
does not prove split-K atomics, more-than-four simdgroups, non-f32 atomics,
arbitrary TileLang/TIRx lowering, external framework DLPack/stream interop,
