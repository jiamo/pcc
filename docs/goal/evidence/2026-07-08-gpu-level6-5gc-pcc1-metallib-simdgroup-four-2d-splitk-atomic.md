# 2026-07-08 five-GC pcc1 Metallib Simdgroup Four-2D Split-K Atomic Evidence
...
simdgroup/tensorcore-style prebuilt `.metallib` workload using split-K atomic
accumulation.
...
A: 16x16 f16
B: 16x16 f16
C: 16x16 f32, lowered as device atomic_float*
split_k: 2 z-axis threadgroups
threads: 128
```
...
`split_k0 = split_k_index * 8u`, stage per-simdgroup C tiles, and accumulate
with:
...
f16 A/B payloads, zero-initializes the f32 C payload before launch, calls
...
gtimeout 180s env -u LC_ALL \
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_level6_classifier_accepts_complete_pcc1_five_gc_simdgroup_four_2d_splitk_atomic_metallib_matrix \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_four_2d_splitk_atomic_metallib_lifetime_real_or_skipped \
  -rs
```
...
workload: the first four-simdgroup 2D split-K atomic GEMM, covering two
z-axis split-K launches over a 16x16 output with explicit C zero
initialization and f32 atomic accumulation. It does not prove edge/tail
split-K atomic metallib, transpose_AB split-K atomic metallib, more-than-four
