# TileLang A/B Parallel Tile-Copy Runtime Evidence
...
- The broader TileLang coverage now explicitly proves `T.Parallel(...)`
  metadata on both legal GEMM staging copies:
  - A global-to-shared tile copy into `A_shared`.
  - B global-to-shared tile copy into `B_shared`.
- The A+B tile-copy variant preserves `parallel_extents` and `parallel_vars`
  through import and TIRx/plain-TIR freeze.
- CPU reference and Metal source validation accept both staging copies only
  when the parallel extents match the destination tile shape.
- Runtime-source Metal execution now covers the A+B `T.Parallel` tile-copy
...
This proves only the legal global-to-shared A/B tile-copy staging form for the
current strict scalar tiled GEMM subset. It does not prove arbitrary
`T.Parallel` loop bodies, non-copy parallel loops, C/output parallel copies,
`T.vectorized`, real split-k atomic accumulation, TileLang/TVM pass execution,
`.air/.metallib` production, pcc1-native GPU launch, or whole-program GPU
execution.
