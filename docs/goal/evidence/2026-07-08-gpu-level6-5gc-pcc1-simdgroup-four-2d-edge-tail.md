# 2026-07-08 GPU Level-6 Five-GC pcc1 Simdgroup Four-2D Edge/Tail Evidence

## Summary

`GPU-P0-SIMDGROUP-TENSORCORE-GEMM` now has Level-6 five-GC proof for the first
2D opt-in Metal simdgroup GEMM edge/tail tile:
`M=15,N=15,K=9`, `block_M=16,block_N=16,block_K=8`, and `threads=128`.

The test builds one pcc1 no-libpython executable for the four-2D edge/tail
...
writes odd-sized f16 A(15,9) and B(9,15) payloads byte-by-byte, launches the
...
gtimeout 900s env -u LC_ALL \
  ...
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_four_2d_edge_tail_lifetime_real_or_skipped -rs
```
