# 2026-07-08 five-GC pcc1 Metallib Simdgroup Eight-N Transpose-AB Split-K Atomic Edge/Tail Evidence
...
eight-simdgroup prebuilt `.metallib` workload combining transpose_A+transpose_B
operand layout, split-K atomic accumulation, and non-divisible M/N/K edge-tail
guards.

The covered workload is an eight-simdgroup N-extended GEMM with f16 A/B inputs
and f32 C output:

```text
A: 17x15 f16, read as transposed A
B: 31x17 f16, read as transposed B
C: 15x31 f32, lowered as device atomic_float*
...
split eight simdgroups as `simdgroup_tile_m = simdgroup_idx / 4u` and
...
gtimeout 180s env -u LC_ALL \
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_level6_classifier_accepts_complete_pcc1_five_gc_simdgroup_eight_n_transpose_ab_splitk_atomic_edge_tail_metallib_matrix \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_simdgroup_eight_n_transpose_ab_splitk_atomic_edge_tail_metallib_lifetime_real_or_skipped \
  -rs
```
...
workload: the first eight-simdgroup transpose_A+transpose_B split-K atomic
edge/tail GEMM, covering four z-axis split-K launches over a 15x31 output and
...
sixteen/thirty-two or arbitrary simdgroup tiling on the `.metallib` path,
