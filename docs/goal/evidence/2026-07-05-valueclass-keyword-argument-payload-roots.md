# Evidence: valueclass keyword-argument payload roots
...
The direct valueclass payload-root contract now covers typed function calls
where a pointer-bearing valueclass payload is supplied through a keyword
argument. The test exercises both a constructor temporary passed as
`touch_holder(h=Holder(...))` and an existing direct payload local passed as
`touch_holder(h=selected)`, with explicit collections before mutation and
readback under all five GC backends.

This is a coverage-strengthening slice only. No implementation change was
needed: the existing call-argument resolution and direct payload frame-root
...
# 140 passed in 23.28s
```

## Open Boundary

`AUD-P0-GC-SLOT-VISITOR` remains `DONE_WEAK`. The covered direct-payload set now
includes keyword-argument calls, but broader value payload slots, remaining
