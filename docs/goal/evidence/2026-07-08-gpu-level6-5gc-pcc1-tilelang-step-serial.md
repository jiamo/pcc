# 2026-07-08 GPU Level-6 Five-GC pcc1 TileLang T.serial Step Evidence

## Summary

`GPU-P1-BROADER-TILELANG-TIRX-PASSES` now has Level-6 five-GC proof for the
same pcc1-native stepped `T.serial(start, end, step)` scalar tiled GEMM shape
covered by the Level-5 slice.

The test builds one pcc1 no-libpython executable for the runtime-source
TileLang stepped-serial package, then runs that same executable with
...
gtimeout 1500s env -u LC_ALL \
  ...
  uv run pytest -q -n0 \
  tests/gpu_hardware/test_metal_5gc_lifetime_real.py::test_gpu_level6_five_gc_tilelang_step_serial_lifetime_real_or_skipped -rs
```
...
stepped `T.Pipelined`, not arbitrary nested loop lowering, not performance
